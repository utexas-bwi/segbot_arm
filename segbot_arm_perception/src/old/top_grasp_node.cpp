...
float computeTopMostPoint(Eigen::Vector4f centroid, PointCloudT::Ptr in){
	float topmost_z=centroid(2);
...
int main (int argc, char** argv)
{
	...
	//debugging publisher
	ros::Publisher cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("top_grasp_node/cloud", 10);

	//button position publisher
	ros::Publisher pose_pub = nh.advertise<geometry_msgs::PoseStamped>("top_grasp_node/pose", 10);
	...
	while (!g_caught_sigint && ros::ok())
	{
		...
		if (new_cloud_available_flag && cloud_mutex.try_lock ())    // if a new cloud is available
		{
			...
			for (unsigned int i = 0; i < clusters_on_plane.size(); i++){
				...
				float topmost_z = computeTopMostPoint(centroid, clusters_on_plane.at(i));
				ROS_INFO("TopMost_z: %f",topmost_z);
				dist_to_spec_pos.push_back(std::sqrt(((centroid(0) + 0.19) * (centroid(0) + 0.19)) + ((centroid(1) - 0.14) * (centroid(1) - 0.14)) + ((topmost_z - 0.95) * (topmost_z - 0.95)))); 
			}
            ...
            if ((min_index >= 0) &&
				...
				float topmost_z_final = computeTopMostPoint(centroid, clusters_on_plane.at(min_index));
				pose_i.position.x=centroid(0);
				pose_i.position.y=centroid(1);
				pose_i.position.z=topmost_z_final;
