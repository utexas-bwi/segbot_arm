...
#define MAX_DISTANCE_TO_PLANE 0.08
...
int main(int argc, char **argv) {
	...
	{
		ROS_ERROR("Failed to call tabletop_perception service");
		return 1;
	}
	...
	listenForGrasps(30.0);
		...
		ROS_INFO("wait for response at 30Hz");
	...
	for (unsigned int i = 0; i < grasp_commands.size(); i++){
		...
		double diff = angular_difference(grasp_commands.at(i).approach_pose.pose.orientation, grasp_commands.at(i).grasp_pose.pose.orientation);
		ROS_INFO("difference");
